...
if(self::getExtension($class) == "php"){
...
foreach($classarray AS $service){
  echo "<tr>
	...
	<td>POST</td>
	<td>Call this service to retreive $service when setting up a terminal for the first time. For Technical support, contact ".self::config("admin_email")."</td>
</tr>"; 	
}
echo '</tbody></table>';
}
...
echo "<li>Url: ".self::config("website_root")."api/$version/$class/$method.xml</li>"; 
echo '</ul>';
